...
from discrete.diffusion_utils import SumExceptBatchKL, NLL, SumExceptBatchMetric, cal_identify_TF_gene
...
class GaussianDiffusion1D(nn.Module):
    ...
    def p_losses(self, E, noisy_data, loss_method='celoss', calAUC=True):
        ...
        # noisy_data['time_tensor_tonode'] = time_tensor_tonode
        # noisy_data['batch'] = batch
        # noisy_data['ALLedge_index'] = ALLedge_index
    ...
    def forward(self, data, *args, **kwargs):
        ...
        """
        dense_data, node_mask, max_num_nodes = network_preprocess.to_dense(data.x, data.edge_index, data.edge_attr,
                                                                           data.batch, max_num_nodes=self.max_num_nodes)
        ...
        '''提取 存储知识库中的边索引和权重（经过sc-PC修剪），并存入noisy_data以引导Transformer'''
        noisy_data['E_reg'] = None
        if data.get('crop_Reg_edge_index', None) is not None:
            Reg_dense_data, Reg_node_mask, _ = network_preprocess.to_dense(data.x,
                                                                           data.crop_Reg_edge_index,
                                                                           data.crop_Reg_edge_attr,
                                                                           data.batch,
                                                                           max_num_nodes=self.max_num_nodes,
                                                                           discrete=False)

            E_reg = Reg_dense_data.mask(Reg_node_mask).E
            noisy_data['E_reg'] = E_reg

        '''提取 LLM中的基因嵌入信息，并存入noisy_data以Transformer中的基因特征'''
        noisy_data['gene_LLM_embedding'] = None
        if data.get('LLM_embedding', None) is not None:
            LLM_dense_data, LLM_node_mask, _ = network_preprocess.to_dense(data.LLM_embedding,
                                                                           data.llm_edge_index,
                                                                           data.llm_edge_attr,
                                                                           data.batch,
                                                                           max_num_nodes=self.max_num_nodes,
            ...
            LLM = LLM_dense_data.mask(LLM_node_mask)
            noisy_data['gene_LLM_embedding'] = LLM.X
            noisy_data['gene_E_LLM_embedding'] = LLM.E
        loss, pred, AUC = self.p_losses(E, noisy_data)
        return loss, AUC
    ...
    def validation_step(self, data):
        ...
        if data.y is None:
            data.y = torch.empty(data.num_graphs, 0)
        noisy_data = self.apply_noise(X, E, data.y, node_mask)
        loss, pred = self.p_losses(E, noisy_data)
    ...
    def test_step(self, testdata, TrueData=None, show=True, seed=None):
        ...
        # 导入数据库关联作为引导
        E_reg = None
        if testdata.get('crop_Reg_edge_index', None) is not None:
            Reg_dense_data, _, _ = network_preprocess.to_dense(testdata.x,
                                                                           testdata.crop_Reg_edge_index,
                                                                           testdata.crop_Reg_edge_attr,
                                                                           testdata.batch,
                                                                           max_num_nodes=self.max_num_nodes,
                                                                           discrete=False)
            E_reg = Reg_dense_data.mask(node_mask).E
        # 导入LLM作为引导
        LLME = None
        LLMX = None
        if testdata.get('LLM_embedding', None) is not None:
            LLM_dense_data, _, _ = network_preprocess.to_dense(testdata.LLM_embedding,
                                                                           testdata.llm_edge_index,
                                                                           testdata.llm_edge_attr,
                                                                           testdata.batch,
                                                                           max_num_nodes=self.max_num_nodes,
                                                                           discrete=False)

            LLM = LLM_dense_data.mask(node_mask)
            LLMX = LLM.X
            LLME = LLM.E
        ...
        for t_int in pbar2:
            ...
            noisy_data = {'X_t': dense_X, 'trueE': E, 'E_t': E, 'y_t': y, 's': s_norm, 't': t_norm, 't_int': t_array,
                          's_int': s_array, 'node_mask': node_mask, 'E_reg': E_reg, 'gene_LLM_embedding': LLMX, 'gene_E_LLM_embedding':LLME}
            noisy_data['y_t'] = noisy_data['t'].float()
