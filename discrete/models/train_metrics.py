...
class TrainLossDiscrete(nn.Module):
    ...
    def forward(self, masked_pred_E, nosoft_pred_E, true_E,CEloss=False):
        ...
        if CEloss == True:
            loss_train = F.cross_entropy(pred, target)
        else:
            loss_train = F.binary_cross_entropy(pred, target, reduction='mean')
            # loss_train = focal_loss(flat_pred_E[:, 1], flat_true_E[:, 1], alpha=0.25, gamma=1.2)
        return loss_train
