...
class NetworkGPT:
    def __init__(self, args):
        ...
        printlabel3 = '                               ' + str(self.use_PCA) + '                ' \
            ...
            self.lr)
        print(printlabel3)
        if self.args.net_key_par['Flag_reg']:
            print(f'use Database: {self.args.database}.')
        if self.args.net_key_par['Flag_llm']:
            print(f'use LLM: {self.args.LLM_filepath}.')
    ...
    def load_test_data(self, test_filename, num = 0):
        # If test_filename is a pickle result, then the i-th expression and network (if any) will be extracted
        # If test_filename is a csv or xlsx expression spectrum, then num will not work
        if not is_csv_or_xlsx(test_filename):
            testdata = create_batch_dataset_simu(filename=test_filename,
                                                 ...
                                                 Cnum=self.Cnum,
                                                 k=self.k,
                                                 Adddatabse=self.args.Adddatabse,
                                                 theta={'PMI': self.args.theta_PMI,
                                                        'PPC': self.args.theta_PPC,
                                                        'database': self.args.database,
                                                        'LLM_metric': self.args.net_key_par['LLM_metric']}
                                                 ).to(self.device)
        else:
            testdata = create_batch_dataset_from_cancer(filepath=test_filename,
                                                        ...
                                                        lim=self.max_nodes,
                                                        Adddatabse=self.args.Adddatabse,
                                                        theta={'PMI': self.args.theta_PMI, 'PPC': self.args.theta_PPC,
                                                               'database': self.args.database, 'LLM_metric': self.args.net_key_par['LLM_metric']},
                                                        LLM_filepath=self.args.LLM_filepath)
    ...
    def load_data(self, train_filename, n_train=200, n_test=[1000, 1010]):
        '''
            train_filename: Please enter the training data file name. If it is simulated data, please enter the pickled file containing the list file (each list contains 'net' and 'exp', please refer to the simulated data for details)
            n_train : Enabled in simulated data training, this parameter is the number of networks trained, for example n_train = 200
            n_test : Enabled in simulated data training, this parameter is the number of networks tested, for example n_test = [1000,1010]
        '''
        # load data
        if not is_csv_or_xlsx(train_filename):
            ...
            for test_i in range(n_test[0], n_test[1]):
                testdata = create_batch_dataset_simu(filename=train_filename,
                                                     num=test_i,
                                                     test=True,
                                                     device=self.device,
                                                     metacell=self.metacell,
            ...
            if self.test_pathway is not None:
                ...
                REGtruelabel, REGnode_mask, _ = network_preprocess.to_dense(testdata.x, testdata.crop_Reg_edge_index,
                                                                      testdata.crop_Reg_edge_attr,
                                                                      training=True, max_num_nodes=testdata.x.shape[0])
                REGtruelabel_discrete = REGtruelabel.mask(REGnode_mask, collapse=True)
                REGtruelabel_discrete = REGtruelabel_discrete.E.squeeze(0)
                result = Evaluation(y_pred=REGtruelabel_discrete.flatten(),
                                    y_true=truelabel_discrete.flatten())['AUC']
                resultau = Evaluation(y_pred=REGtruelabel_discrete.flatten(),
                                    y_true=truelabel_discrete.flatten())['AUPR']
                print(f'Test knowledge-based AUC is {result} AUPRC is {resultau}')
    ...
    def setup_model(self):
        ...
        self.graph_data_loader = DataLoader(self.data, batch_size=self.batch_size, shuffle=False)

        # create network

        self.hidden_mlp_dims = {'X': self.MLP_base_node * 4,
                                'LLM': self.MLP_base_node * 4,
        ...
        LLM_hidden = self.data['LLM_embedding'].shape[-1] if 'LLM_embedding' in self.data is not None else 1
        self.input_dims = {'X': self.data.num_features,
                           'LLM': LLM_hidden,
                           'E': 1,
                           'y': 1}
        self.output_dims = {'X': self.data.num_features,
                            'E': 2,
                            'y': 1}
    ...
    def train(self, train_filename, n_train=200, n_test=[1000, 1010]):
        ...
        if self.save_dir is not None:
            final_filename = f"{self.save_label}_DigNet_{self.timestamp}.pth"
    ...
    def validation(self, epoch):
        ...
        printf = "  --- Epoch  %.4f Final+std： Test AUC: %.4f+%.4f  AUPR: %.4f+%.4f  AUPRM: %.4f+%.4f F1: %.4f+%.4f" % (
                ...
                np.mean(results['AUPR_norm']), np.std(results['AUPR_norm']),
        ...
        if np.mean(results['AUC'][-self.rep_num:]) > self.best_mean_AUC:
            ...
            self.best_mean_AUC_file = self.checkpoint_dir + f'_checkpoint_{self.timestamp}_epoch{epoch + 1}.pth'
            print(f"  --- Best AUC is {self.best_mean_AUC}")
            print(f"  --- Best best_AUC_file is {self.best_mean_AUC_file}")
            self.printf = printf
